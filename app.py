...
if check_password():
    ...
    if raw_patient_df is not None and mapping_df is not None:
        ...
        def interactive_dashboard_fragment(patient_df, mapping_df, geo_data, min_date, max_date, data_source):
            ...
            # --- FAST FOLIUM MAP RENDERING WITH SMOOTH SPINNER LOADER ---
            with st.container(border=True):
                ...
                if geo_data:
                    # Smooth loading transition wrapper for map computation & rendering
                    with st.spinner("Updating spatial intelligence map..."):
